  ...
  if (routineId) {
    try {
      ...
      if (routine?._id) {
        ...
        dayOfWeek.forEach((day, index) => {
          ...
          routine.classes
            ...
            .forEach((info, index2) => {
              ...
              doc.text(
                textConversion(info.subjectName, maxTextLength),
                classX + 10,
                classY + 25
              );

              doc.text(
                textConversion(info.teacherName, maxTextLength),
                classX + 10,
                classY + 40
              );

              doc.text(
                "RN-" + textConversion(info.roomNumber, maxTextLength - 4),
                classX + 10,
                classY + 55
              );
              let startTime = moment(info.startTime).format("hh:mm A");
              let endTime = moment(info.endTime).format("hh:mm A");
        ...
        res.setHeader(
          "Content-Disposition",
          "inline; filename=class_routine.pdf"
        );
